...
export class SysAdminByDomainComponent {
  ...
  one_checked: { [env: number]: boolean } = {
    1: false,
    2: false,
    3: false,
    4: false,
  ...
  all_checked: { [env: number]: boolean } = {
    1: false,
    2: false,
    3: false,
    4: false,
  ...
  getSysAdminByDomain(domain_id: any, all: boolean): void {
    ...
    this.http
      .get('http://localhost:5050/api/sysadminbydomain/' + domain_id)
      .subscribe(
        (data: any) => {
          data.domains.sort((a: any, b: any) => {
    ...
    for (const login of this.logins) {
      ...
      for (const user of this.users) {
        if (
          ...
          user.domainId === this.selected_domain.domainId
        ) {
      ...
      if (Object.keys(tab_users).length > 0) {
        ...
        var user_env: { [key: number]: boolean } = {
          1: false,
          2: false,
          3: false,
          4: false,
        ...
        for (const user of Object.keys(tab_users)) {
          for (var i = 1; i < 7; i++) {
            if (
              tab_users[user].environment === i &&
              tab_users[user].sysAdmin === true
            ) {
              user_env[i] = true;
            }
          }
  ...
  check(loginId: any, env: any): void {
    ...
    if (this.login_users[loginId][2][env]) {
      dialogConfig.data = {
        user: this.login_users[loginId][2][env],
        name: this.login_users[loginId][0],
      };
      ...
      dialogRef.afterClosed().subscribe((result) => {
        if (
          ...
          if (this.login_users[loginId][2][env].sysAdminEndDate != null)
            this.show_calendar[loginId][env] = true;
          else this.show_calendar[loginId][env] = false;
  ...
  saveSysAdmin(): void {
    ...
    for (const loginId of Object.keys(this.login_users)) {
      if (this.login_users[loginId].length > 0) {
        for (const env of Object.keys(this.login_users[loginId][2])) {
          if (
            this.login_users[loginId][2][env].userId ===
              '99999999-9999-9999-9999-999999999999' &&
            this.login_users[loginId][2][env].sysAdmin === true
          ) {
            const request = this.http.put(
              'http://localhost:5050/api/sysadminbydomain',
              this.login_users[loginId][2][env]
            );
  ...
  allChecked(env: any): boolean {
    for (const loginId of Object.keys(this.login_users)) {
      if (this.login_users[loginId].length == 0) return false;
      if (this.login_users[loginId].length > 0) {
        if (this.login_users[loginId][2][env] == null) return false;
      }
    }
    return true;
  }
